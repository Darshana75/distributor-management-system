...
public function storeCustomerOrderInvoice($invoice_number, $customer_name, $orderdate, $find_customer_name, $total_quantity, $orderQuantity, $price, $totalPrice, $pro_name, $pid, $subtotal, $prev_due, $netTotal, $paidBill, $dueBill, $payMethode) {

	$pre_stmt = $this->pdo->prepare("INSERT INTO `invoice`(`invoice_number`, `customer_id`, `customer_name`, `order_date`, `sub_total`, `pre_cus_due`, `net_total`, `paid_amount`, `due_amount`, `payment_type`) VALUES (?,?,?,?,?,?,?,?,?,?)");
	...
	$pre_stmt->bindParam(6, $prev_due, PDO::PARAM_STR);
	$pre_stmt->bindParam(7, $netTotal, PDO::PARAM_STR);
	$pre_stmt->bindParam(8, $paidBill, PDO::PARAM_STR);
	$pre_stmt->bindParam(9, $dueBill, PDO::PARAM_STR);
	$pre_stmt->bindParam(10, $payMethode, PDO::PARAM_STR);
	...
	if ($invoice_no != null) {
		...
		// update customer transition totals
